...
## 🦊 노트
### 접근
파티가 입력 순대로 주어지면, set을 이용해서 파티에 진실을 아는 사람이 있으면 그 파티원들을 모조리 set에 추가하는 식으로 해결할 수 있지만<br/>
입력 순대로 주어지지 않기 때문에 다른 알고리즘으로 해결해야 함<br/>
서로 다른 두 원소가 같은 집합에 속했는 지 알아야 할 때 -> union find를 이용하자
